...
global $product, $post, $wc_atc_bll_public;
...
<form class="cart grouped_form" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data'>
	...
	<?php if ( $quantites_required && $show_add_to_cart_button ) : ?>
		...
		<?php 
			$grouped_button_link_single = $wc_atc_bll_public->wc_atc_bll_get_product_urls("grouped_button_link_single");
			$wc_atc_opn_lnks_nw_tb = $wc_atc_bll_public->wc_atc_bll_get_settings( 'wc_atc_opn_lnks_nw_tb');
			$new_tab = "";
			if($wc_atc_opn_lnks_nw_tb == "1"){
				$new_tab = "target='_blank'";
			}
			if(empty($grouped_button_link_single)){
				?>
					<button type="submit" class="single_add_to_cart_button button alt"><?php echo esc_html( $product->single_add_to_cart_text() ); ?></button>
				<?php
			} else {
				?>
					<a href="<?php echo $grouped_button_link_single; ?>" <?php echo $new_tab; ?> class="single_add_to_cart_button button alt" data-product-id="<?php echo esc_attr( $product->get_id() ); ?>"><?php echo esc_html( $product->single_add_to_cart_text() ); ?></a>
				<?php
			}
		?>
